...
class Evaluator(object):
	...
	def make_env(env_name, record_video=False, **kwargs):
		...
		is_dataset = 'Dataset' in env_name

		if is_dataset:
			env.load_levels(levels=kwargs.get('levels', None),
							labels=kwargs.get('labels', None),
							next_level_on_reset=kwargs.get('next_level_on_reset', 'random'))
	...
	def _init_parallel_envs(self, env_names, num_processes, device=None, record_video=False, **kwargs):
		...
		split_levels = [None]*num_processes
		split_labels = [None]*num_processes
		if kwargs.get('levels', None) is not None:
			assert len(env_names) == 1, "Levels can only be split for a single environment"
			num_levels_per_env = np.ceil(len(kwargs['levels'])/num_processes).astype(int)
			split_levels = [kwargs['levels'][i*num_levels_per_env:(i+1)*num_levels_per_env] for i in range(num_processes)]
			kwargs.pop('levels')
			if kwargs.get('labels', None) is not None:
				split_labels = [kwargs['labels'][i*num_levels_per_env:(i+1)*num_levels_per_env] for i in range(num_processes)]
				kwargs.pop('labels')

		for env_name in env_names:
			make_fn = [partial(Evaluator.make_env, env_name, record_video,
							   levels=split_levels[p], labels=split_labels[p], **kwargs) for p in range(num_processes)]
	...
	def evaluate(self,
		...
		for env_name, venv in self.venv.items():
			returns = []
			solved_episodes = 0
			level_labels = [] if 'Dataset' in env_name else None
				...
				for i, info in enumerate(infos):
					if 'episode' in info.keys():
						...
						if level_labels is not None:
							level_label = venv.remote_attr('previous_level_label', flatten=True)[i]
							if eval_level_once and level_label in level_labels:
								continue
		...
		for env_name in self.env_names:
			...
			if accumulator == 'mean':
				...
				stats[f"level_labels:{env_name}"] = env_level_labels[env_name]
