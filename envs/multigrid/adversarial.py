...
EDITOR_ACTION_SPACES = {
  ...
  'walls_none_lava_floor_agent_goal': {
    0: '-',
    1: '.',
    2: 'l',
    3: 'f',
...
class AdversarialEnv(multigrid.MultiGridEnv):
  ...
  def __init__(self,
    ...
    Args:
      ...
      editor_actions: The set of actions the adversary/level editor can take.
        See EDITOR_ACTION_SPACES.
      deterministic_start_state: If True, the agent will always start in the
        same orientation.
      agent_start_dir: The direction the agent will face at the start of the
        episode. Only used if deterministic_start_state is True.
    """
    ...
    self.deterministic_start_state = deterministic_start_state
    if deterministic_start_state:
  ...
  def compute_metrics(self):
    ...
    self.compute_resistance_distance()
    self.compute_navigable_nodes()
  ...
  def mutate_level(self, num_edits=1):
    ...
    """
    num_tiles = int((self.width-2)*(self.height-2))
    ...
    for loc, a in zip(edit_locs, actions):
      ...
      if a == '-':
        ...
        self.put_obj(minigrid.Lava(), x, y)
        self.wall_locs.append((x-1, y-1))
        self.n_clutter_placed += 1
        free_mask[y-1,x-1] = False
      elif a == 'f':
        self.put_obj(multigrid.Floor(), x, y)
        free_mask[y-1,x-1] = True
    ...
    if self.goal_pos is None:
      ...
      x += 1
      y += 1
      self._clean_loc(x,y) #handles cleaning floor or other "texture" objects
      self.put_obj(minigrid.Goal(), x, y)
      self.goal_pos = np.array((x,y))
  ...
  def remove_wall(self, x, y):
    ...
    if obj is not None and (obj.type == 'wall' or obj.type == 'lava'):
      self.grid.set(x, y, None)

  def _count_walls(self):
    wall_mask = np.array(
      [1 if isinstance(x, minigrid.Wall) or isinstance(x, minigrid.Lava) else 0 for x in self.grid.grid],
  ...
  def compute_resistance_distance(self):
    ...
    if self.passable:
      ...
      graph = self.graph.subgraph(
        nx.node_connected_component(self.graph, (self.agent_start_pos[0] - 1, self.agent_start_pos[1] - 1)))
      self.resistance_distance = nx.resistance_distance(
          graph,
          (self.agent_start_pos[0] - 1, self.agent_start_pos[1] - 1),
          (self.goal_pos[0] - 1, self.goal_pos[1] - 1))
    else:
      self.resistance_distance = 0.

  def compute_navigable_nodes(self):

    if self.agent_start_pos is None or self.goal_pos is None:
      return

    # Check the number of reachable nodes from the agent's start position. Remember
    # to subtract 1 due to outside walls existing in the Grid, but not in the
    # networkx graph.
    graph = self.graph.subgraph(
      nx.node_connected_component(self.graph, (self.agent_start_pos[0] - 1, self.agent_start_pos[1] - 1)))
    self.navigable_nodes = graph.number_of_nodes()
  ...
  def reset_random(self):
    ...
    clutter_type = [a for a in self.editor_actions if a in ['l', 'f', '-']]
    clutter_idx = np.random.randint(0, len(clutter_type), n_clutter)
    clutter = [clutter_type[i] for i in clutter_idx]

    for i in range(n_clutter):
      if clutter[i] == 'l':
        self.place_obj(minigrid.Lava(), max_tries=100)
      elif clutter[i] == 'f':
        self.place_obj(multigrid.Floor(), max_tries=100)
      elif clutter[i] == '-':
        self.place_obj(minigrid.Wall(), max_tries=100)
      else:
        raise ValueError(f'Invalid clutter type {clutter[i]}')
...
class CaveEscapeAdversarialEnv(AdversarialEnv):
  ...
  def __init__(self,
    ...
    super().__init__(
      editor_actions='walls_none_lava_floor_agent_goal',
      n_clutter=n_clutter,
      resample_n_clutter=resample_n_clutter,
      size=size,
      agent_view_size=agent_view_size,
      max_steps=max_steps,
      goal_noise=goal_noise,
      random_z_dim=random_z_dim,
      choose_goal_last=choose_goal_last,
  ...
  def step_adversary(self, loc):
    raise NotImplementedError("CaveEscapeAdversarialEnv does not support step_adversary(), "
                              "for random level generation set --use_reset_random_dr=True")
...
class CaveEscapeGoalLastAdversarialEnv(CaveEscapeAdversarialEnv):
  def __init__(self, fixed_environment=False, seed=None):
    super().__init__(seed=seed, max_steps=250, fixed_environment=fixed_environment,
                     choose_goal_last=True)

class CaveEscapeVariableBlocksAdversarialEnv(CaveEscapeAdversarialEnv):
  def __init__(self, fixed_environment=False, seed=None):
    super().__init__(n_clutter=60, resample_n_clutter=True, seed=seed, max_steps=250,
                     fixed_environment=fixed_environment)

class CaveEscapeGoalLastVariableBlocksAdversarialEnv(CaveEscapeAdversarialEnv):
  def __init__(self, fixed_environment=False, seed=None):
    super().__init__(n_clutter=60, resample_n_clutter=True, seed=seed, max_steps=250,
                     fixed_environment=fixed_environment, choose_goal_last=True)

class CaveEscapeGoalLastEmptyAdversarialEnv(CaveEscapeAdversarialEnv):
  def __init__(self, fixed_environment=False, seed=None):
    super().__init__(n_clutter=0, seed=seed, max_steps=250, fixed_environment=fixed_environment,
                     choose_goal_last=True)

class CaveEscapeAdversarialS45Env(CaveEscapeAdversarialEnv):
  def __init__(self, fixed_environment=False, seed=None):
    super().__init__(size=45, seed=seed, max_steps=2250, fixed_environment=fixed_environment)
...
register.register(
    env_id='MultiGrid-CaveEscapeAdversarialEnv-v0',
    entry_point=module_path + ':CaveEscapeAdversarialEnv',
    max_episode_steps=250,
)

register.register(
    env_id='MultiGrid-CaveEscapeGoalLastAdversarialEnv-v0',
    entry_point=module_path + ':CaveEscapeGoalLastAdversarialEnv',
    max_episode_steps=250,
)

register.register(
    env_id='MultiGrid-CaveEscapeVariableBlocksAdversarialEnv-v0',
    entry_point=module_path + ':CaveEscapeVariableBlocksAdversarialEnv',
    max_episode_steps=250,
)

register.register(
    env_id='MultiGrid-CaveEscapeGoalLastVariableBlocksAdversarialEnv-v0',
    entry_point=module_path + ':CaveEscapeGoalLastVariableBlocksAdversarialEnv',
    max_episode_steps=250,
)

register.register(
    env_id='MultiGrid-CaveEscapeGoalLastEmptyAdversarialEnv-v0',
    entry_point=module_path + ':CaveEscapeGoalLastEmptyAdversarialEnv',
    max_episode_steps=250,
)

register.register(
    env_id='MultiGrid-CaveEscapeAdversarialS45Env-v0',
    entry_point=module_path + ':CaveEscapeAdversarialS45Env',
