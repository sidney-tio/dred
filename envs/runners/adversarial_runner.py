...
from level_replay import LevelSampler, LevelStore, MinigridDatasetSampler, LevelGenerator
from util import \
    array_to_csv, \
    is_discrete_actions, \
    get_obs_at_index, \
    set_obs_at_index, \
    make_level_generator_args
...
class AdversarialRunner(object):
    ...
    def __init__(
        ...
        self.is_dred = args.ued_algo == 'dred'
        ...
        if args.dataset_path is not None:
            assert args.env_name.startswith('MultiGrid'), \
                "Loading a dataset of levels is currently only available for MultiGrid environments."
            self.dataset_sampler = MinigridDatasetSampler(args.dataset_path, reload_level_encodings=True)
        if plr_args:
            ...
            if self.is_dred and args.level_editor_method == 'generative_model':
                level_generator_args = make_level_generator_args(args, self.device, self.level_store.data_info)
                self.level_generator = LevelGenerator(**level_generator_args)

            if args.use_dataset:
                assert self.dataset_sampler is not None, "Must specify a dataset path to use a dataset."
                plr_args = self._init_plr_to_dataset(plr_args)
    ...
    def _get_env_stats_multigrid(self, agent_info, adversary_agent_info):
        ...
        resistance_distances = agent_info.get(
            'resistance_distances', self.venv.get_resistance_distance())
        navigable_nodes = agent_info.get(
            'navigable_nodes', self.venv.get_navigable_nodes())

        shortest_path_length = np.mean(shortest_path_lengths)
        # only compute the mean resistance of the solvable environments
        resistance_distance = np.mean(resistance_distances[resistance_distances != 0.0])
        navigable_nodes = np.mean(navigable_nodes)
        ...
        solved_resistances = np.array(resistance_distances)[solved_idx]
        solved_resistance = np.mean(solved_resistances) if len(solved_resistances) > 0 else 0
        ...
        stats = {
            ...
            'solved_resistance': solved_resistance,
            'navigable_nodes': navigable_nodes,
        }
    ...
    def _get_active_levels(self):
        ...
        if self.use_byte_encoding:
            return [self.level_store.encode_bytes(x) for x in self.ued_venv.get_encodings()]
        elif is_multigrid:
    ...
    def _init_plr_to_dataset(self, plr_args):
        args = self.args
        levels = self.dataset_sampler.train_levels
        labels = self.dataset_sampler.train_labels
        if self.is_dred and self.level_generator is not None:
            latent_params = self.level_generator.compute_latent_distribution_parameters(self.dataset_sampler,
                                                                                        split='train')
            from_dataset = [True] * len(levels)
        else:
            latent_params = None
            from_dataset = None
            labels = None
        if self.use_byte_encoding:
            levels = [self.level_store.encode_bytes(x) for x in levels]
        seeds = self.level_store.insert(levels, latents=latent_params, labels=labels, from_dataset=from_dataset)
        plr_args['seeds'] = seeds
        plr_args['rho'] = len(seeds) / args.level_replay_seed_buffer_size
        return plr_args

    def _update_plr_with_current_unseen_levels(self, latents=None, parent_seeds=None, labels=None,
                                               from_dataset=None):
        args = self.args
        levels = self._get_active_levels()
        self.current_level_seeds = self.level_store.insert(levels, latents=latents, parent_seeds=parent_seeds,
                                            labels=labels, from_dataset=from_dataset)
        dataset_labels = [self.level_store.seed2datasetlabel.get(s, -1) for s in self.current_level_seeds]
        ...
        self._update_level_samplers_with_external_unseen_sample(
            self.current_level_seeds, solvable=passable, dataset_labels=dataset_labels)

    def _update_level_samplers_with_external_unseen_sample(self, seeds, solvable=None, dataset_labels=None):
        ...
        for level_sampler in level_samplers:
            level_sampler.observe_external_unseen_sample(seeds, solvable, dataset_labels)
    ...
    def _get_latent_pairs(self):
        num_pairs = self.args.num_level_pairs_for_interpolation
        levels, labels = self._sample_train_levels_from_dataset(num_pairs * 2)
        seeds = [self.level_store.datasetlabel2seed[lbl] for lbl in labels]
        latent_params_batch = self.level_store.get_latent_distribution_parameter_batch(seeds)
        pair_ids = list(reversed([(i, 2 * num_pairs - 1 - i) for i in range(num_pairs)]))
        return latent_params_batch, seeds, labels, pair_ids

    def _sample_replay_decision(self):
        return self._default_level_sampler.sample_replay_decision()

    def _sample_train_levels_from_dataset(self, num_levels):
        return self.dataset_sampler.sample_train_levels(num_levels)

    def _sample_levels_from_level_generator(self):
        base_latent_params_batch, base_seeds, base_labels, pair_ids = self._get_latent_pairs()
        levels, latent_params_batch, parent_seeds, labels = \
            self.level_generator.generate_levels(base_latent_params_batch, pair_ids,
                                                 base_seeds=base_seeds, base_labels=base_labels)
        latent_params = [(latent_params_batch[0][i].cpu(), latent_params_batch[1][i].cpu())
    ...
    def agent_rollout(self,
        ...
        if is_env:
            if edit_level: # Get mutated levels
                if args.level_editor_method == 'random': #ACCEL Editor
                    levels = [self.level_store.get_level(seed) for seed in fixed_seeds]
                    self.ued_venv.reset_to_level_batch(levels[:self.ued_venv.num_envs])
                    self.ued_venv.mutate_level(num_edits=num_edits)
                    if fixed_seeds is not None:
                        fixed_seeds = fixed_seeds[:self.ued_venv.num_envs]
                    if args.use_dataset:
                        labels = [self.level_store.seed2label.get(seed, None) for seed in fixed_seeds]
                    else:
                        labels = None
                    self._update_plr_with_current_unseen_levels(parent_seeds=fixed_seeds, labels=labels)
                elif args.level_editor_method == 'generative_model': # Generative Model Editor
                    levels, latents, parent_seeds, labels = self._sample_levels_from_level_generator()
                    self.ued_venv.reset_to_level_batch(levels[:self.ued_venv.num_envs])
                    self._update_plr_with_current_unseen_levels(latents=latents[:self.ued_venv.num_envs],
                                                                parent_seeds=parent_seeds[:self.ued_venv.num_envs],
                                                                labels=labels[:self.ued_venv.num_envs])
                return
            if level_replay or args.use_dataset: # Get replay levels
                self.current_level_seeds = [level_sampler.sample(replay_decision=level_replay) for _ in range(args.num_processes)]
    ...
    def run(self):
        ...
        if level_replay and self.args.level_editor_method != 'generative_model':
            sampled_level_info = {
                'level_replay': True,
        ...
        if level_replay and edit_level:
            ...
            if self.base_levels == 'batch':
                ...
                if args.num_processes >= 4:
                    ...
                    fixed_seeds = [env_info[x.item()] for x in easy] * int(np.ceil(args.num_processes/4))
                    fixed_seeds = fixed_seeds[:args.num_processes]
                else:
                    # take top 1
