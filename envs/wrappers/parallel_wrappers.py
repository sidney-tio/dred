        ...
        while True:
            ...
            if cmd == 'step':
                ...
                if data is None:
                    ...
                    remote.send([env.render(mode='level', tile_size=data) for env in envs])
...
class SubprocVecEnv(VecEnv):
    ...
    def get_images(self, tile_size=None):
        self._assert_not_closed()
        for remote in self.remotes:
            remote.send(('render', tile_size))
...
class ParallelAdversarialVecEnv(SubprocVecEnv):
    ...
    def set_agent_start_dir(self, orientations, index=None):
        return self.remote_attr('set_agent_start_dir', data=orientations, flatten=True, index=index)
    ...
    def get_resistance_distance(self):
        return self.remote_attr('resistance_distance', flatten=True)

    def get_navigable_nodes(self):
        return self.remote_attr('navigable_nodes', flatten=True)
